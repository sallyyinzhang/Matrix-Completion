...
OE_extend = function(PI,  visible_matrix, true_matrix, r, query_budge, condition_limit){
  ...
  return(list( error = (base::norm(true_matrix, type = "F") - base::norm(recover2, type = "F"))/base::norm(true_matrix, type = "F"),
              recover = recover,
              query = query))
}
...
true_matrix = x
visible_matrix = OE_visible(true_matrix = x, visibility = 0.35)
qerror = c()
for (i in 0:15)
{query_budge = round(length(true_matrix)*(i/100))
r=2
condition_limit = 3
PI = OE_order(visible_matrix = visible_matrix, r, adjustment = FALSE)
qerror[i+1] = OE_extend(PI,visible_matrix, true_matrix, r, query_budge, condition_limit)[[1]]
}
save(qerror, file = "querry_error.RData")
